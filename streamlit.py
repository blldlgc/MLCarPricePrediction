...
def main():
    ...
    st.sidebar.title('Araç Fiyat Tahmin Uygulaması')
    ...
    if selected_page == "-":
        ...
        st.markdown(
            ...
            """)
        st.info(
            "Tahmin yapmak,istatistlikleri görüntülemek için sol tarafta bulunan menüyü kullanınız.")
...
def predict():
    ...
    prediction_value = create_prediction_value(selected_km,selected_brand,selected_year,selected_engine,selected_gear,selected_fuel,selected_drive)
...
def create_prediction_value(kilometre, marka, model_yili, motor_hacmi, vites_tipi, yakit_turu, cekis_tipi):
    res = pd.DataFrame(data=
                       {'Kilometre': [kilometre], 'Marka': [marka], 'Model_Yili': [model_yili],
                        'Motor_Hacmi': [motor_hacmi], 'Vites_Tipi': [vites_tipi],
                        'Yakit_Turu': [yakit_turu], 'Cekis_Tipi': [cekis_tipi]})
    return res
